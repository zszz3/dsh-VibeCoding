...
English | [中文](https://github.com/deepseek-ai/deepseek-harness/blob/master/.agents/notes/implemented/process/2026-07-27-worktree-local-lefthook.zh.md)

## Problem

Every `pnpm install` runs the root [`postinstall`](https://github.com/deepseek-ai/deepseek-harness/blob/master/package.json), whose [`install-lefthook.mjs`](https://github.com/deepseek-ai/deepseek-harness/blob/master/scripts/install-lefthook.mjs) invokes `lefthook install --force`. Linked Git worktrees otherwise share the common repository's default hooks directory, so an install in any worktree can rewrite hooks used by every other worktree.
