...
English | [中文](https://github.com/deepseek-ai/deepseek-harness/blob/master/.agents/notes/implemented/process/2026-07-19-web-styling-system.zh.md)

> Division of labor: this RFC fixes the framework and constraints (rarely changes); [docs/web-styling.md](https://github.com/deepseek-ai/deepseek-harness/blob/master/docs/web-styling.md) is the living spec (authoritative token values, the coding-rule checklist, the deviation record — it evolves with the implementation). Token changes and new rules go there; only changes to the framework itself come back here (overturning it requires a new RFC).
