...
> 这是 [examples.md](examples.md) 的中文对照,供阅读方便。**英文原文是权威版本**,判断时以原文为准。
>
> **本文里带 `Leaked:` / `Fixed:` / `Keep:` 标记的英文句子是「病例标本」,一律保留原文未译** —— 翻译它们会让读者失去识别这些措辞的能力。每条标本后面的中文是对它的解释。

从 2026-08 那次全仓库清理及其各轮评审中提炼出来。用它们来识别背后的原则,而不是当成文本模板照搬。本文刻意引用了泄漏的措辞作为校准材料 —— [召回探针](recall-batteries.md)会排除本 skill 自己的目录,而这里的措辞不构成在别处这么写的许可。
