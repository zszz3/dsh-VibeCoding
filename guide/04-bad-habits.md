...
**用关键词探测 AI 的输出来判断成功。** 「回答里出现了『完成』就算过」,这等于把判卷权交给考生。

**放任「以前 / 现在」式的叙述。** 单看一处很自然,攒半年就成了考古现场。改法是只写当前状态;确实需要记一次变更,那属于决策记录或事故复盘,不属于代码注释。

**清理得过头。** 这是个真实的反向风险:AI 接到「清理」任务,很容易把 issue 号、豁免理由、实测数据一起删掉。所以规范必须同时写清「什么不该删」——下一篇会看到 dsh 是怎么处理这一点的。
