...
没有 `status:` 字段,没有数据库,也没有管理工具。状态和种类都编码在路径里:
...
于是「所有被否掉的简化提案」就是 `ls rejected/simplification/`,而**状态变化就是一次 `git mv`**。
...
| 文件夹 | 装什么 | 进出规则 |
|---|---|---|
| `proposed/` | 实施前先评审的提案。允许一部分已经动工,不是必须一行没写 | 落地时**重写**成现在时再移走;过时的提案是**否决**,不是归档 |
| `implemented/` | 已经交付的决策,记着做了什么决定、否掉了什么 | 跟着代码更新事实,同一次改动里改完;但不改决策本身 |
| `rejected/` | 讨论之后被否掉的提案 | 只在 `Status:` 行补一句理由,然后冻住;理由不再能防错时整篇删掉 |

下面几条是日常真正会用到的:

**「更新事实但不改决策」最容易读错。** 可以改的是路径、包名、配置键、默认值这类事实;不能改的是当初决定了什么、否掉了什么。拿那篇 zstd 举例:它记着「不引外部原生 zstd 依赖,因为 Node 版本底线已经自带编解码器」。哪天那个 API 改名了,就地把名字改对;但**不能**把结论改成「现在引入了 X 库」——那是新决定。如果允许改决策,这个文件就退化成「我们当前的想法」的镜子,而它存在的全部价值恰恰是记住当时为什么这么选。

**不能把一篇记录改成相反的决定。** 要反悔就新写一篇取代它,两篇互相链接。直接改掉,「我们曾经反悔过」这件事就彻底消失了。详见[已落地记录的维护规则](../.agents/notes/implemented/AGENTS.md)。

**rejected 留不留,只问一件事:这篇理由还能不能挡住一个诱人且影响大的错误?** 不按年龄,不按数量,不设配额。下面那篇依赖审计的否决清单一直留着,就是因为「把手写的换成成熟库」这个建议每个人都会提。删的时候要删「三文件组」:英文、中文对照、以及记录两份配对状态的 `.i18n.yaml` 边车,三个一起删,不留孤儿。(本仓库只收了英文,这条不适用。)

**文件名里的日期是首次提出的时间,不是完成时间。** 6 月提出、8 月落地,搬进 `implemented/` 之后文件名还是 6 月。这样同一个主题不会因为落地就跳到另一个时间点,顺序稳定。

**记录之间引用必须用相对 Markdown 链接**,写 `[topic](../../implemented/architecture/2026-…-….md)`,不能写「见架构那篇」,也不能写编号。两个理由:死链能被脚本查出来;文件在生命周期目录之间搬动时,链接能被一起修好。

**别加中央索引。** 目录树加仓库搜索就是工作清单。这条也有记录,理由见下面第一个例子。

未来指导价值低的已落地记录,移进单独的 `archived/` 冻结树。进去之后不编辑、不翻译、不重排、不移动,也不能拿它当当前行为的依据。
...
|---|---|
| `feature` | 面向用户或模型的新能力 |
| `bug-fix` | 修缺陷,或补上事故复盘发现的缺口 |
| `simplification` | 不增加能力的前提下,移除代码、行为或对外范围 |
| `architecture` | 交付源码的结构性决策:包之间怎么关联、运行时词汇是什么 |
| `process` | 代码周边的工具、策略、工作流——门禁、包管理器、vendor 化——不涉及运行时行为 |
| `testing` | 测试基础设施与策略 |

`architecture` 和 `process` 的界线:**前者管我们交付的源码,后者管围绕源码的工具和流程。**

**没有 `refactor` 这一类,是故意的。** 它和 `simplification` 重叠,而后者的判别标准「可观察行为是否改变」已经覆盖了它。少一个模糊类别,就少一处每次归档都要纠结的地方。

这六个是封闭集合,写在 `scripts/agent-note-tree.ts` 里,分类门禁拒绝任何别的文件夹。想加一类,得同时改那份代码和制度文档——[第三篇](03-gates.md)讲的门禁在这里的作用,就是让「随手建个新目录」这件事直接失败。
...
三种写法:`Status: proposed`、`Status: implemented`、`Status: rejected — <一行理由>`。**不带日期,不带括注。** 只有 rejected 那行带内容,因为翻开一篇被否的记录,你要的就是那句结论。日期在文件名里,其余在 git 里。这种「一个事实只有一个家」的克制,正是门禁能检查这个文件的前提。
