...
| # | 机制 | 一句话 | 本库位置 |
|---|---|---|---|
| ① | **常驻规则** | 每个 session 都自动读到的规则索引,只放结论 + 链接 | [templates/AGENTS.md](templates/AGENTS.md) |
| ② | **决策记忆** | 每个非平凡改动配一篇决策记录,**必写「否掉了什么」** | [agent-notes/](agent-notes/README.md) |
| ③ | **机器门禁** | 规则焊成脚本,三道关卡逐级变严,检查取代信任 | [docs/gates.md](docs/gates.md) |
| ④ | **坏习惯规范** | 治「思维链泄漏」「自我报告」「含糊措辞」 | [skills/dsh-trim-cot-leakage/](skills/dsh-trim-cot-leakage/SKILL.md) |
| ⑤ | **判断固化** | 把「AI 容易做错且不易发现」的判断写成可自动触发的 skill | [skills/](skills/README.md) |
| ⑥ | **隔离并行 + 互审** | worktree 隔离、stacked PR、AI 审 AI 且不许表演性同意 | [skills/dsh-code-review/](skills/dsh-code-review/SKILL.md) |
...
5. 把高频活做成 skill(抄 [templates/SKILL.md](templates/SKILL.md),参考 [skills/](skills/README.md))
6. 多 AI 隔离并行 + 互审
...
docs/
  ...
  adoption-roadmap.md     六步采用路线,每步的产出与依赖关系
...
agent-notes/
  README.md               决策记录制度:生命周期、分类、格式、取代规则 †
  implemented-notes-rules.md  已落地记录怎么保持与现实一致 †
templates/
  AGENTS.md               常驻规则骨架(可直接抄)
  agent-note-implemented.md / agent-note-proposed.md
  SKILL.md                skill 骨架
skills/                   11 个可复用工作流(原文移植)†
  README.md               索引:每个 skill 固化了哪条判断 + 可迁移度
```

† = 从 DeepSeek Harness 原文移植,保留英文原貌(它们是实际工作文档);本库新写的导读与方法论层为中文。

## 来源与授权

内容源自 [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness),MIT 授权。移植时保留了原文规则,仅修正跨仓库链接:能对应到本库的指向本库,dsh 专有的(具体决策记录、双语机制、内部脚本)指向上游 GitHub,读者仍可点进去看原始理由。
