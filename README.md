...
[DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 用来跟 AI 协同开发的那套文件,加一篇说明怎么用它们的文章。

文件是 dsh 的原件,放在原本的路径上,内容没改,只修了跨仓库的链接。文章就是这篇 README。

需要注意:这些文件里的命令、目录、技术选择说的是 dsh 那个仓库,不是这里。抄走的时候要换成自己项目的事实。
